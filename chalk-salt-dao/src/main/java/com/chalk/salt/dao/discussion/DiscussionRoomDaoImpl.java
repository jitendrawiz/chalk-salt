...
public class DiscussionRoomDaoImpl implements DiscussionRoomDao{
	...
	public List<TopicStatisticsDto> getTopicsCount(String classId) throws Exception {
		final String sqlQuery = "SELECT DISTINCT "
				...
				+ " WHERE cst_discussion_topics.subject_id = cst_class_subject_mapping.subject_id AND cst_discussion_topic_comments.delete_status=1 "
				+ " )AS comments, "
				+ " (SELECT "
				...
				+ " WHERE cst_discussion_topics.subject_id = subjects.subject_id AND cst_discussion_topic_comments.delete_status=1 ) AS lastModifiedDate, "
				...
				+ " INNER JOIN cst_discussion_topic_comments ON cst_discussion_topics.discussion_topic_id = cst_discussion_topic_comments.discussion_topic_id "
				+ " WHERE cst_discussion_topics.subject_id = subjects.subject_id AND cst_discussion_topic_comments.delete_status=1 "
